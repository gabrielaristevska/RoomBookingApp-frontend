...
const useAccommodations = () => {
    ...
    const onAdd = useCallback((data) => {
        accommodationRepository.add(data)
            .then(() => {
                console.log("Successfully added new accommodation");
                fetchAccommodations();
            })
            .catch((error) => console.log(error));
    }, [fetchAccommodations]);
    ...
    const onEdit = useCallback((id, data) => {
        accommodationRepository.edit(id, data)
            .then(() => {
                console.log(`Successfully edited accommodation with id:${id}`);
                fetchAccommodations();
            })
            .catch((error) => console.log(error));
    }, [fetchAccommodations]);
    ...
    const onDelete = useCallback((id) => {
        accommodationRepository.delete(id)
            .then(() => {
                console.log("Successfully deleted accommodation with id:${id}");
                fetchAccommodations();
            })
            .catch((error) => console.log(error));
    }, [fetchAccommodations]);
